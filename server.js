...
// + CORS & WebSocket CORS priekš https://thezone.lv (Render).
// =====================
...
// ---- Atļautie frontenda domēni (pieliec, ja vajag vēl kādu subdomēnu)
const FRONT_ORIGINS = [
  "https://thezone.lv",
...
// Render/Proxy ieteikums
app.set("trust proxy", 1);

// CORS: dinamisks Allow-Origin + preflight atbilde
app.use((req, res, next) => {
  ...
  // Ja izmanto cookies/credentials, atkomentē nākamo rindu un pieskaņo klientu
  // res.header("Access-Control-Allow-Credentials", "true");
  if (req.method === "OPTIONS") return res.sendStatus(204);
  next();
});

// Papildus, ja vēlies izmantot cors() helperi (nekaitē)
app.use(
  cors({
    origin: (origin, cb) => {
      if (!origin) return cb(null, true); // ļauj server-side pingus
      return cb(null, FRONT_ORIGINS.includes(origin));
    },
    methods: ["GET", "POST", "OPTIONS"],
    allowedHeaders: ["Content-Type", "Authorization"],
    credentials: false, // ja vajag cookies, ieliec true un saskaņo klientu
  })
);
...
// Socket.IO ar CORS, kas saskan ar lapu
const io = new Server(httpServer, {
  cors: {
    origin: FRONT_ORIGINS,
    methods: ["GET", "POST"],
    // credentials: true, // ieslēdz tikai, ja tiešām izmanto
  },
  transports: ["websocket", "polling"], // Render atbalsta abus
  // allowEIO3: true, // ja būtu vajadzīgs legacy klients
});
...
function initDeck(mode) {
  ...
  const trumpCard = full[0];
  const deck = full.slice(1);
...
function tableRanks(room) {
  ...
  for (const pr of room.table) {
    if (pr.attack) s.add(pr.attack.r);
    if (pr.defend) s.add(pr.defend.r);
  }
  return s;
}
...
function cleanRoomId(x) {
  x = String(x || "").trim();
  if (!SAFE_ROOM.test(x)) throw new Error("Nederīgs istabas ID");
  return x;
}
function cleanNick(x) {
  x = String(x || "").trim();
  if (!SAFE_NICK.test(x)) throw new Error("Nederīgs segvārds");
  return x;
}
...
function drawOne(room) {
  ...
  if (room.trumpAvailable) {
    room.trumpAvailable = false;
    return room.trumpCard;
  }
  return null;
}
...
function endBoutDefended(room) {
  for (const pr of room.table) {
    room.discard.push(pr.attack);
    if (pr.defend) room.discard.push(pr.defend);
  }
  room.table = [];
  dealUpToSix(room);
...
function endBoutTook(room) {
  const def = room.players[room.defender];
  for (const pr of room.table) {
    def.hand.push(pr.attack);
    if (pr.defend) def.hand.push(pr.defend);
  }
  room.table = [];
  dealUpToSix(room);
...
function schedule(room, fn, d) {
  clearBotTimer(room);
  room.botTimer = setTimeout(fn, d);
}
function botShouldPlay(room) {
  if (room.phase !== "attack") return false;
  const a = room.players[room.attacker],
    d = room.players[room.defender];
  return a?.isBot || d?.isBot;
}
function botThinkDelay(room) {
  return room.botStepMs || rand(BOT_STEP_MIN, BOT_STEP_MAX);
}
function msg(room, text) {
  room.chat.push(text);
  if (room.chat.length > 200) room.chat.splice(0, room.chat.length - 200);
  io.to(room.id).emit("message", text);
  emitState(room);
}

function botOneStep(room) {
  if (room.phase !== "attack") return false;
  const aI = room.attacker,
    dI = room.defender;
  const A = room.players[aI],
    D = room.players[dI];
  const trump = room.trumpSuit,
    ranks = room.ranks;
  ...
  if (D?.isBot) {
    ...
    if (open.length) {
      const i = open[0],
        atk = room.table[i].attack;
      const cand = D.hand
        .filter((c) => canCover(atk, c, trump, ranks))
        .sort((x, y) => rankValue(x.r, ranks) - rankValue(y.r, ranks));
      if (cand.length) {
        const card = cand[0];
        D.hand.splice(
          D.hand.findIndex((c) => c.id === card.id),
          1
        );
        ...
        if (allCovered && room.passes.size === activePlayers(room).length - 1) {
          const defender = room.players[room.defender];
          bumpStats(defender.cid || defender.id, defender.nick, (s) => {
            s.cleanDefends = (s.cleanDefends || 0) + 1;
          });
  ...
  if (A?.isBot) {
    ...
    if (card) {
      A.hand.splice(
        A.hand.findIndex((c) => c.id === card.id),
        1
      );
...
function runBot(room) {
  ...
  try {
    ...
    console.error("BOT step error:", e);
    did = false;
  }
  emitState(room);
...
function replaceWithBot(room, idx) {
  ...
  room.players[idx] = {
    ...
    cid: `bot-${Math.random().toString(36).slice(2, 5)}`,
    nick: "BOT",
    hand: left.hand || [],
    isBot: true,
    ready: true,
...
app.get("/api/leaderboard", (req, res) => {
  ...
  const rows = asSortedArray(src, key)
    .map((s) => ({
      cid: s.cid,
      nick: s.lastNick,
      wins: s.wins || 0,
      clean: s.cleanDefends || 0,
...
io.on("connection", (socket) => {
  ...
  if (cid && sessions.has(cid)) {
    ...
    if (room) {
      ...
      if (p) {
        ...
        socket.join(room.id);
        if (room.hostId === oldSocketId) room.hostId = socket.id; // atjauno hostu
        emitState(room);
      }
    }
  }
  ...
  socket.on("createRoom", ({ roomId, nickname, deckMode }) => {
    ...
    try {
      ...
      nickname = cleanNick(nickname || "Spēlētājs");
    } catch (e) {
  ...
  function startGame(room, botStepMs) {
    ...
    room.trumpAvailable = trumpAvailable;
    room.ranks = ranks;
  ...
  socket.on("playAttack", ({ roomId, card }) => {
    ...
    withRoomLock(room, () => {
      try {
        ...
        if (idx < 0) throw new Error("Nav spēlētāja");
        const me = room.players[idx];
        if (me.spectator) throw new Error("Skatītājs nevar uzbrukt");
        validateAttackAllowed(room, idx, card);
        const hi = me.hand.findIndex((c) => c.id === card.id);
        if (hi < 0) throw new Error("Tev tādas kārts nav");
  ...
  socket.on("playAttackMany", ({ roomId, cards }) => {
    ...
    withRoomLock(room, () => {
      try {
        ...
        if (room.lastAction.cards?.length >= 3) {
          room.comboCandidate = { attackerId: room.lastAction.playerId, count: room.lastAction.cards.length, boutIndex: room.boutCount };
        }
  ...
  socket.on("playDefend", ({ roomId, attackIndex, card }) => {
    ...
    withRoomLock(room, () => {
      try {
        ...
        if (idx < 0) throw new Error("Nav spēlētāja");
        const me = room.players[idx];
        if (me.spectator) throw new Error("Skatītājs nevar aizsegt");
        if (idx !== room.defender) throw new Error("Tikai aizsargs drīkst aizsegt");

        const pair = room.table[attackIndex];
        if (!pair || pair.defend) throw new Error("Nepareizs pāris");
        const hi = me.hand.findIndex((c) => c.id === card.id);
        if (hi < 0) throw new Error("Tev tādas kārts nav");
        const realCard = me.hand[hi];
  ...
  socket.on("takeCards", ({ roomId }) => {
    ...
    withRoomLock(room, () => {
      try {
        ...
        if (idx < 0) throw new Error("Nav spēlētāja");
        const me = room.players[idx];
        if (me.spectator) throw new Error("Skatītājs nevar ņemt");
  ...
  socket.on("pass", ({ roomId }) => {
    ...
    withRoomLock(room, () => {
      try {
        ...
        if (idx < 0) throw new Error("Nav spēlētāja");
        const me = room.players[idx];
        if (me.spectator) throw new Error("Skatītājs nevar pasēt");

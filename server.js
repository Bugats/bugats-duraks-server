...
const equalRankToAnyOnTable=(pairs,r)=>{
  if(pairs.length===0) return true;
  const ranks=new Set(); for(const p of pairs){ ranks.add(p.attack.rank); if(p.defend) ranks.add(p.defend.rank); }
...
function pushState(room){
  const base={ room:room.code, phase:room.phase, trump:room.trump, stockCount:room.stock.length, stack:room.stack, attackerId:room.attackerId, defenderId:room.defenderId, turnId:room.turnId };
  const ids=Object.keys(room.players);
  ids.forEach(pid=>{
    const opp=ids.find(x=>x!==pid);
    const st={ ...base, meId:pid, hands:{[pid]:room.hands[pid]}, opponent: opp ? {id:opp, count:room.hands[opp].length} : {id:null,count:0} };
...
function take(room, defenderId){
  ...
  room.hands[defenderId].push(...all); room.stack=[];
  // iedala (sāk uzbrucējs), lomas nemainās
  for(const pid of [room.attackerId, defenderId]) dealToSix(room,pid);
  room.turnId = room.attackerId; room.phase="attack";
  pushState(room);
}
...
io.on("connection",(socket)=>{
  ...
  socket.on("createRoom",({nick,deckSize,solo})=>{
    ...
    const room={ code, players:{[socket.id]:{id:socket.id,nick:nick||"Viesis"}}, hands:{[socket.id]:[]}, stock:deck, discard:[], stack:[], phase:"wait", trump:null, attackerId:null, defenderId:null, turnId:null, botId: solo ? "BOT_"+code : null };
    dealToSix(room,socket.id);
    rooms[code]=room; socket.join(code); socket.data.room=code;

    if(solo){
      const bid=room.botId; room.players[bid]={id:bid,nick:"BOT"}; room.hands[bid]=[]; dealToSix(room,bid);
      startRound(room); pushState(room); io.to(socket.id).emit("room",code);
    }else{
      pushState(room); io.to(socket.id).emit("room",code); io.to(socket.id).emit("info","Istaba izveidota: "+code);
    }
  });
  ...
  socket.on("joinRoom",({nick,room:code})=>{
    ...
    room.players[socket.id]={id:socket.id,nick:nick||"Viesis"}; room.hands[socket.id]=room.hands[socket.id]||[]; dealToSix(room,socket.id);
  ...
  socket.on("chat",(m)=>{ const code=socket.data.room; if(!code) return; const room=rooms[code]; if(!room) return; io.to(code).emit("chat", `${room.players[socket.id]?.nick||"Viesis"}: ${m}`); });
  ...
  socket.on("attack",({cards})=>{
    ...
    if(open+cards.length>defCount) return socket.emit("errorMsg","Nevar uzbrukt ar vairāk kā aizstāvis var nosist.");

    for(const c of cards){
      const i=hand.findIndex(x=>x.rank===c.rank && x.suit===c.suit);
      if(i<0) return socket.emit("errorMsg","Kārts nav rokā.");
      hand.splice(i,1); room.stack.push({attack:c,defend:null});
    }
  ...
  socket.on("defend",({pairIndex,card})=>{
    ...
    // ja visi pāri nosisti — uzbrucējs var beigt metienu
    if(room.stack.length && room.stack.every(p=>p.defend)){
      room.turnId=room.attackerId;
    }
    pushState(room);
  ...
  socket.on("take",()=>{ const code=socket.data.room; if(!code) return; const room=rooms[code]; if(!room) return; if(room.defenderId!==socket.id) return; take(room,socket.id); });
  socket.on("passAdd",()=>{ const code=socket.data.room; if(!code) return; const room=rooms[code]; if(!room) return; pushState(room); });

  socket.on("disconnect",()=>{
    const code=socket.data.room; if(!code) return; const room=rooms[code]; if(!room) return;
    if(room.players[socket.id]){
      delete room.players[socket.id]; delete room.hands[socket.id];
      const ids=Object.keys(room.players).filter(x=>!room.botId || x!==room.botId);
      if(ids.length===0) delete rooms[code]; else pushState(room);
    }
  });
});
